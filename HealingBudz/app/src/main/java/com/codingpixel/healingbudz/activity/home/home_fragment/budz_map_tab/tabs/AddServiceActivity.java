...
public class AddServiceActivity extends AppCompatActivity implements APIResponseListner {
    ...
    private HealinBudEditText mServiceName;
...
//                    mediaPagerParent.setVisibility(View.VISIBLE);
            ...
            public void onItemClick(Object obj, int pos, int type) {
                ...
                if (postMedia.isVideo()) {
                    ...
                    Utility.launchActivity(AddServiceActivity.this, MediPreview.class, false, b);
                }
            }
    ...
    private void setEditView() {
        mChargeName.setText(editObject.getCharges());
        mServiceName.setText(editObject.getName());
        if (editObject.getImage_path().length() > 4) {
            temp = new BudzFeedNewPostMediaViewPagerAdapter.NewPostMedia(editObject.getImage_path(), false);
            temp.url = editObject.getImage_path();
            ...
            public void onClick(View v) {
                ...
                if (isValid()) {
                    ...
                    try {
                        ...
                        object.put("service_name", mServiceName.getText().toString().trim());
                        object.put("service_charges", mChargeName.getText().toString().trim());
                        object.put("path", "");
                        if (editObject != null) {
                            if (editObject.getImage_path().length() > 4)
...
//                        parameter.put("rating");
                        parameter.put("service_name");
                        parameter.put("service_charges");
                        if (editObject != null) {
                            parameter.put("id");
                        }
                        JSONArray values = new JSONArray();
                        values.put(BudzMapHomeFragment.budz_map_item_clickerd_dataModel.getId());
                        values.put(mServiceName.getText().toString().trim());
                        ...
                        if (attachments.length() > 2 && attachments.toString().contains("com.healingbudz.android")) {
                            new UploadFileWithProgress(v.getContext(), true, URL.add_budz_service, attachments.toString(), "image", values, parameter, null, AddServiceActivity.this, APIActions.ApiActions.add_product).execute();
                        } else {
        ...
        mPhotoUpload.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mediaAdapter.getCount() < 1) {
                    ...
                    Utility.launchActivityForResult(AddServiceActivity.this, HBCameraActivity.class, b, Flags.PICTURE_CAPTUREING_REQUEST);
                } else {
                    CustomeToast.ShowCustomToast(AddServiceActivity.this, "Max 1 Picture only..", Gravity.TOP);
                }
            }
        });
    }
    ...
    public boolean isValid() {
        if (mServiceName.getText().toString().trim().length() == 0) {
            CustomeToast.ShowCustomToast(this, "Service Name Required!", Gravity.TOP);
            return false;
        }
...
//                    uploadMedia(temp);
    ...
    private void initView() {
        ...
        mServiceName = findViewById(R.id.name_service);
    ...
    public void onRequestSuccess(String response, APIActions.ApiActions apiActions) {
        if (apiActions == wall_post_img_upload) {
            ...
            CustomeToast.ShowCustomToast(AddServiceActivity.this, "Media Uploaded!", Gravity.TOP);
