...
public class GiraffePlayer {
    ...
    public GiraffePlayer(final Activity activity, final View view) {
        try {
        ...
        screenWidthPixels = view.getContext().getResources().getDisplayMetrics().widthPixels;
        $=new Query((Activity) view.getContext());
...
//                        Toaster.show("download rate:" + extra);
        ...
        final GestureDetector gestureDetector = new GestureDetector((Activity) view.getContext(), new PlayerGestureListener());
        ...
        orientationEventListener = new OrientationEventListener((Activity) view.getContext()) {
            @Override
            public void onOrientationChanged(int orientation) {
                if (orientation >= 0 && orientation <= 30 || orientation >= 330 || (orientation >= 150 && orientation <= 210)) {
                    //竖屏
                    if (portrait) {
                        ((Activity) view.getContext()).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_SENSOR);
        ...
        if (fullScreenOnly) {
            ((Activity) view.getContext()).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
        }
        portrait=getScreenOrientation()==ActivityInfo.SCREEN_ORIENTATION_PORTRAIT;
        initHeight=view.findViewById(R.id.app_video_box).getLayoutParams().height;
        hideAll();
        if (!playerSupport) {
            showStatus(view.getResources().getString(R.string.not_support));
        }
    }
